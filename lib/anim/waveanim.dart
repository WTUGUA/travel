...
//动画第一次进入页面就会启动
  ...
  void setStart(){
    setState(() {
      ...
      //initAnimation(_percentage ,_percentage);
      animationController.forward();
      print("动画开始");
    });
  }
...
class BezierCurvePainter extends CustomPainter{
  ...
  //  double radius=min(size.width, size.height)/2;
    ...
    canvas.saveLayer(new Rect.fromLTRB(0.0, 0.0, size.width+40, size.height+200), layerPaint);
    //绘制淡颜色的海浪
    canvas.save();
    ...
    canvas.saveLayer(new Rect.fromLTRB(0.0, 0.0,  0.0, 0.0), layerPaint);
...
//控制高度
  ...
  void setStart(){
    bezierCurveState.setStart();
  }
  void setStop(){
    bezierCurveState.setStop();
  }
}
